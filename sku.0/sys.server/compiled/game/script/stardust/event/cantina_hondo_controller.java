...
public class cantina_hondo_controller extends script.base_script
{
    public cantina_hondo_controller()
    {
    }
    ...
    public int startBosskEvent(obj_id self, dictionary params) throws InterruptedException
    {
        ...
        obj_id bossk = create.object("hondo", new location(43.8f, 0.1f, 0.5f, getLocation(self).area, getCellId(self, "foyer1")));
        if (isIdValid(bossk))
        {
            setInvulnerable(bossk, true);
	    setName(bossk, "Hondo Ohnaka");
            setObjVar(bossk, "bhcelebs.actor_role", "bossk");
            attachScript(bossk, "event.bh_event.cantina_actor_hondo");
        ...
        if (isGod(objSpeaker))
        {
            if (strText.equals("forcebosskevent"))
            {
                ...
                if (invasionActive == 0)
                {
                    messageTo(self, "startBosskEvent", null, 1, false);
                    setObjVar(self, "bhcelebs.invasion_active", 1);
                    announceStatusToPlayers(self, "pirate_business_is_good_business");
                }
                else 
                {
