...
public class kreel extends script.base_script
{
    public kreel()
    {
    }
    public static final String c_stringFile = "conversation/kreel";
    public static final String OBJ_VAR_BASE = "kreel.";
    public static final String SITH_APPRENTICE = OBJ_VAR_BASE + "sith";
    public boolean kreel_defaultCondition()
    {
        return true;
    }
    public boolean kreel_language_condition(obj_id npc, obj_id player) throws InterruptedException
    {
        return hasSkill(player, "social_language_basic_comprehend");
    }
    public boolean kreel_sithFriend_condition(obj_id player, obj_id npc) throws InterruptedException
    {
        float sithFaction = factions.getFactionStanding(player, "sith_shadow");
        return sithFaction >= 2500;
    }
    public boolean kreel_phase1_condition(obj_id npc, obj_id player)
    {
        return hasSkill(player,"class_forcesensitive_phase1_novice");
    }
    public boolean kreel_phase2_condition(obj_id npc, obj_id player)
    {
        return hasSkill(player,"class_forcesensitive_phase2_novice");
    }
    public boolean kreel_phase3_condition(obj_id npc, obj_id player)
    {
        return hasSkill(player,"class_forcesensitive_phase3_novice");
    }
    public boolean kreel_phase4_condition(obj_id npc, obj_id player)
    {
        return hasSkill(player,"class_forcesensitive_phase4_novice");
    }
    public boolean kreel_credits_condition(obj_id player, obj_id npc) throws InterruptedException
    {
        return (money.hasFunds(player, money.MT_TOTAL, smuggler.TIER_4_GENERIC_PVP_FRONT_COST));
    }
    public boolean kreel_hasObjVar_condition(obj_id npc, obj_id player)
    {
        return hasObjVar(player, SITH_APPRENTICE);
    }
    public boolean kreel_sith_quest_condition_playerFinishedMainTask(obj_id player, obj_id npc) throws InterruptedException
    {
        return groundquests.isTaskActive(player, "stardust_kreel", "talktokreel");
    }
    public void kreel_sith_signalReward(obj_id player, obj_id npc) throws InterruptedException
    {
        groundquests.sendSignal(player, "stardust_entertainer_kreel_reward");
    }
    public void kreel_action_vendor(obj_id player, obj_id npc) throws InterruptedException
    {
        final dictionary d = new dictionary();
        d.put("player", player);
        messageTo(npc, "showInventorySUI", d, 0, false);
    }
    public void kreel_sith_quest(obj_id player, obj_id npc) throws InterruptedException
    {
        String pTemplate = getSkillTemplate(player);
        groundquests.grantQuest(player, "stardust_sith_kreel");
    }
    public void kreel_bounty_quest(obj_id player, obj_id npc) throws InterruptedException
    {
        money.requestPayment(player, npc, smuggler.TIER_4_GENERIC_PVP_FRONT_COST, "none", null, true);
        groundquests.requestGrantQuest(player, "quest/stardust_kreel_arena", true);
    ...
    public int kreel_handleBranch1(obj_id player, obj_id npc, string_id response) throws InterruptedException
    {
        if (response.equals("seek_trade"))
        {
            ...
            utils.setScriptVar(player, "conversation.kreel_conversation.branchId", 2);
        ...
        else if (response.equals("seek_jedi"))
        {
            ...
            utils.removeScriptVar(player, "conversation.kreel_conversation.branchId");
        ...
        else if (response.equals("seek_balance"))
        {
            ...
            utils.setScriptVar(player, "conversation.kreel_conversation.branchId", 3);
        ...
        else if (response.equals("seek_sith"))
        {
            ...
            utils.setScriptVar(player, "conversation.kreel_conversation.branchId", 4);
    ...
    public int kreel_handleBranch2(obj_id player, obj_id npc, string_id response) throws InterruptedException
    {
        if (response.equals("force_trade"))
        {
            if (kreel_sithFriend_condition(npc, player))
            {
                final string_id message = new string_id(c_stringFile, "npc_offer_trade");

                kreel_action_vendor(player, npc);
    ...
    public int kreel_handleBranch3(obj_id player, obj_id npc, string_id response) throws InterruptedException
    {
        if (response.equals("seek_balance2"))
        {
            if (kreel_phase1_condition(npc, player))
            {
    ...
    public int kreel_handleBranch4(obj_id player, obj_id npc, string_id response) throws InterruptedException
    {
    ...
    public int OnInitialize(obj_id self) throws InterruptedException
    {
        ...
        setName(self, "Sergeant Kreel (an Imperial SCAR Trooper)");
    ...
    public int OnStartNpcConversation(obj_id npc, obj_id player) throws InterruptedException
    {
        ...
        if (kreel_language_condition(npc, player))
        {
            ...
            utils.setScriptVar(player, "conversation.kreel_conversation.branchId", 1);

            npcStartConversation(player, npc, "kreel_conversation", message, responses);
    ...
    {
        if (!conversationId.equals("kreel_conversation"))
        {
            return SCRIPT_CONTINUE;
        }

        final int branchId = utils.getIntScriptVar(player, "conversation.kreel_conversation.branchId");

        if (branchId == 1 && kreel_handleBranch1(player, npc, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        else if (branchId == 2 && kreel_handleBranch2(player, npc, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        else if (branchId == 3 && kreel_handleBranch3(player, npc, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        else if (branchId == 4 && kreel_handleBranch4(player, npc, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
