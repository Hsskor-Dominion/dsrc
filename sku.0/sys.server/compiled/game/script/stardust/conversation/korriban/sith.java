...
public class sith extends script.base_script
{
    public sith()
    {
    }
    ...
    public static final String OBJ_VAR_BASE = "sith.";
    public boolean sith_defaultCondition()
    {
        return true;
    }
    public boolean sith_language_condition(obj_id npc, obj_id player) throws InterruptedException
    {
        return hasSkill(player, "social_language_basic_comprehend");
    }
    public boolean sith_chronicler_condition(obj_id npc, obj_id player) throws InterruptedException
    {
        return hasSkill(player, "class_chronicles_master");
    }
    public boolean sith_sithFriend_condition(obj_id player, obj_id npc) throws InterruptedException
    {
        float sithFaction = factions.getFactionStanding(player, "sith_shadow");
        return sithFaction >= 1000;
    }
    public boolean sith_phase1_condition(obj_id npc, obj_id player)
    {
        return hasSkill(player,"class_forcesensitive_phase1_novice");
    }
    public boolean sith_phase2_condition(obj_id npc, obj_id player)
    {
        return hasSkill(player,"class_forcesensitive_phase2_novice");
    }
    public boolean sith_phase3_condition(obj_id npc, obj_id player)
    {
        return hasSkill(player,"class_forcesensitive_phase3_novice");
    }
    public boolean sith_phase4_condition(obj_id npc, obj_id player)
    {
        return hasSkill(player,"class_forcesensitive_phase4_novice");
    }
    public boolean sith_wanderer_condition(obj_id npc, obj_id player)
    {
        return hasSkill(player,"force_rank");
    }
    public boolean sith_jedi_quest_condition_playerFinishedMainTask(obj_id player, obj_id npc) throws InterruptedException
    {
        return groundquests.isTaskActive(player, "stardust_sith", "talktosith");
    }
    public void sith_action_vendor(obj_id player, obj_id npc) throws InterruptedException
    {
        final dictionary d = new dictionary();
        d.put("player", player);
        messageTo(npc, "showInventorySUI", d, 0, false);
    }
    public void sith_jedi_quest(obj_id player, obj_id npc) throws InterruptedException
    {
        String pTemplate = getSkillTemplate(player);
        groundquests.grantQuest(player, "stardust_sith_meditation");
    }
    public int sith_handleBranch1(obj_id player, obj_id npc, string_id response) throws InterruptedException
    {
        if (response.equals("seek_trade"))
        {
            ...
            utils.setScriptVar(player, "conversation.sith_conversation.branchId", 2);
        ...
        else if (response.equals("seek_sith"))
        {
            ...
            utils.setScriptVar(player, "conversation.sith_conversation.branchId", 4);
        ...
        else if (response.equals("seek_balance"))
        {
            ...
            utils.setScriptVar(player, "conversation.sith_conversation.branchId", 3);
        ...
        else if (response.equals("seek_jedi"))
        {
            ...
            utils.removeScriptVar(player, "conversation.sith_conversation.branchId");
        ...
        else if (response.equals("seek_to_leave_order"))
        {
            ...
            utils.setScriptVar(player, "conversation.sith_conversation.branchId", 5);
    ...
    public int sith_handleBranch2(obj_id player, obj_id npc, string_id response) throws InterruptedException
    {
        if (response.equals("force_trade"))
        {
            if (sith_sithFriend_condition(player, npc))
            {
                final string_id message = new string_id(c_stringFile, "npc_offer_trade");
                sith_action_vendor(player, npc);
    ...
    public int sith_handleBranch3(obj_id player, obj_id npc, string_id response) throws InterruptedException
    {
        if (response.equals("seek_balance2"))
        {
            if (sith_chronicler_condition(npc, player))
            {
                final string_id message = new string_id(c_stringFile, "npc_offer_mission");
                sith_jedi_quest(player, npc);
    ...
    public int sith_handleBranch4(obj_id player, obj_id npc, string_id response) throws InterruptedException
    {
        if (response.equals("seek_jedi2"))
        {
            if (sith_wanderer_condition(npc, player))
            {
            ...
            else if (sith_phase2_condition(npc, player))
            {
    ...
    public int sith_handleBranch5(obj_id player, obj_id npc, string_id response) throws InterruptedException
    {
    ...
    public int OnStartNpcConversation(obj_id npc, obj_id player) throws InterruptedException
    {
        ...
        if (sith_language_condition(npc, player))
        {
            ...
            utils.setScriptVar(player, "conversation.sith_conversation.branchId", 1);

            npcStartConversation(player, npc, "sith_conversation", message, responses);
    ...
    {
        if (!conversationId.equals("sith_conversation"))
        {
            return SCRIPT_CONTINUE;
        }

        final int branchId = utils.getIntScriptVar(player, "conversation.sith_conversation.branchId");

        if (branchId == 1 && sith_handleBranch1(player, npc, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        else if (branchId == 2 && sith_handleBranch2(player, npc, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        else if (branchId == 3 && sith_handleBranch3(player, npc, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        else if (branchId == 4 && sith_handleBranch4(player, npc, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        else if (branchId == 5 && sith_handleBranch5(player, npc, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
