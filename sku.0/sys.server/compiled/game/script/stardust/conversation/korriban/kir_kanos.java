...
public class kir_kanos extends script.base_script
{
    public kir_kanos()
    {
    }
    public static final String c_stringFile = "conversation/kir_kanos";
    public static final String OBJ_VAR_BASE = "kir_kanos.";
    public static final String SITH_APPRENTICE = OBJ_VAR_BASE + "sith";
    public boolean kir_kanos_defaultCondition()
    {
        return true;
    }
    public boolean kir_kanos_language_condition(obj_id npc, obj_id player) throws InterruptedException
    {
    ...
    public boolean kir_kanos_phase2_condition(obj_id npc, obj_id player)
    {
        return hasSkill(player,"class_forcesensitive_phase2_novice");
    }
    public boolean kir_kanos_phase3_condition(obj_id npc, obj_id player)
    {
        return hasSkill(player,"class_forcesensitive_phase3_novice");
    }
    public boolean kir_kanos_phase4_condition(obj_id npc, obj_id player)
    {
        return hasSkill(player,"class_forcesensitive_phase4_novice");
    }
    public boolean kir_kanos_credits_condition(obj_id player, obj_id npc) throws InterruptedException
    {
        return (money.hasFunds(player, money.MT_TOTAL, smuggler.TIER_4_GENERIC_PVP_FRONT_COST));
    }
    public boolean kir_kanos_hasObjVar_condition(obj_id npc, obj_id player)
    {
        return hasObjVar(player, SITH_APPRENTICE);
    }
    public boolean kir_kanos_sith_quest_condition_playerFinishedMainTask(obj_id player, obj_id npc) throws InterruptedException
    {
        return groundquests.isTaskActive(player, "stardust_kir_kanos", "talktokir_kanos");
    }
    public void kir_kanos_sith_signalReward(obj_id player, obj_id npc) throws InterruptedException
    {
        groundquests.sendSignal(player, "stardust_entertainer_kir_kanos_reward");
    }
    public void kir_kanos_action_vendor(obj_id player, obj_id npc) throws InterruptedException
    {
        final dictionary d = new dictionary();
        d.put("player", player);
        messageTo(npc, "showInventorySUI", d, 0, false);
    }
    public void kir_kanos_sith_quest(obj_id player, obj_id npc) throws InterruptedException
    {
        String pTemplate = getSkillTemplate(player);
        groundquests.grantQuest(player, "stardust_sith_kir_kanos");
    }
    public void kir_kanos_bounty_quest(obj_id player, obj_id npc) throws InterruptedException
    {
        money.requestPayment(player, npc, smuggler.TIER_4_GENERIC_PVP_FRONT_COST, "none", null, true);
        groundquests.requestGrantQuest(player, "quest/stardust_kir_kanos_arena", true);
    ...
    public int kir_kanos_handleBranch1(obj_id player, obj_id npc, string_id response) throws InterruptedException
    {
        if (response.equals("seek_trade"))
        {
            ...
            utils.setScriptVar(player, "conversation.kir_kanos_conversation.branchId", 2);
        ...
        else if (response.equals("seek_jedi"))
        {
            ...
            utils.removeScriptVar(player, "conversation.kir_kanos_conversation.branchId");
        ...
        else if (response.equals("seek_balance"))
        {
            ...
            utils.setScriptVar(player, "conversation.kir_kanos_conversation.branchId", 3);
        ...
        else if (response.equals("seek_sith"))
        {
            ...
            utils.setScriptVar(player, "conversation.kir_kanos_conversation.branchId", 4);
    ...
    public int kir_kanos_handleBranch2(obj_id player, obj_id npc, string_id response) throws InterruptedException
    {
        if (response.equals("force_trade"))
        {
            if (kir_kanos_sithFriend_condition(npc, player))
            {
                ...
                kir_kanos_action_vendor(player, npc);
    ...
    public int kir_kanos_handleBranch3(obj_id player, obj_id npc, string_id response) throws InterruptedException
    {
        if (response.equals("seek_balance2"))
        {
            if (kir_kanos_phase1_condition(npc, player))
            {
    ...
    public int kir_kanos_handleBranch4(obj_id player, obj_id npc, string_id response) throws InterruptedException
    {
    ...
    public int OnStartNpcConversation(obj_id npc, obj_id player) throws InterruptedException
    {
        ...
        if (kir_kanos_language_condition(npc, player))
        {
            ...
            utils.setScriptVar(player, "conversation.kir_kanos_conversation.branchId", 1);

            npcStartConversation(player, npc, "kir_kanos_conversation", message, responses);
    ...
    {
        if (!conversationId.equals("kir_kanos_conversation"))
        {
            return SCRIPT_CONTINUE;
        }

        final int branchId = utils.getIntScriptVar(player, "conversation.kir_kanos_conversation.branchId");

        if (branchId == 1 && kir_kanos_handleBranch1(player, npc, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        else if (branchId == 2 && kir_kanos_handleBranch2(player, npc, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        else if (branchId == 3 && kir_kanos_handleBranch3(player, npc, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        else if (branchId == 4 && kir_kanos_handleBranch4(player, npc, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
