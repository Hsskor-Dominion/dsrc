...
public class kyle_katarn extends base_script
{
    public kyle_katarn()
    {
    }
    public static final String c_stringFile = "conversation/kyle_katarn";
    public static final String STF_FILE = "stardust/quest";
    public static final String OBJ_VAR_BASE = "kyle_katarn.";

    public boolean kyle_katarn_language_condition(obj_id npc, obj_id player) throws InterruptedException
    {
        return hasSkill(player, "social_language_basic_comprehend");
    }
    public boolean kyle_katarn_chronicler_condition(obj_id npc, obj_id player) throws InterruptedException
    {
        return hasSkill(player, "class_chronicles_master");
    }
    public boolean kyle_katarn_jediFriend_condition(obj_id player, obj_id npc) throws InterruptedException
    {
        float jediFaction = factions.getFactionStanding(player, "fs_villager");
        return jediFaction >= 50;
    }

    public void kyle_katarn_jedi_quest1(obj_id player, obj_id npc) throws InterruptedException
    {
        String pTemplate = getSkillTemplate(player);
        groundquests.grantQuest(player, "stardust_jedi_academy_kyle1");
    }
    public void kyle_katarn_jedi_quest2(obj_id player, obj_id npc) throws InterruptedException
    {
        String pTemplate = getSkillTemplate(player);
        groundquests.grantQuest(player, "stardust_jedi_academy_kyle2");
    }
    public void kyle_katarn_jedi_quest3(obj_id player, obj_id npc) throws InterruptedException
    {
        String pTemplate = getSkillTemplate(player);
        groundquests.grantQuest(player, "stardust_jedi_academy_kyle3");
    }

    public void kyle_katarn_action_grantHiring(obj_id player, obj_id npc, String reinforcementType) throws InterruptedException {
    ...
    public int kyle_katarn_handleBranch1(obj_id player, obj_id npc, string_id response) throws InterruptedException {
        ...
        if (response.equals("seek_sith")) {
            if (kyle_katarn_jediFriend_condition(npc, player)) {
    ...
    private void handleBranch(obj_id player, String messageKey, String[] responseKeys, int branchId) throws InterruptedException {
        ...
        utils.setScriptVar(player, "conversation.kyle_katarn_conversation.branchId", branchId);

        npcSpeak(player, message);
        npcSetConversationResponses(player, responses);
    }
    public int kyle_katarn_handleBranch2(obj_id player, obj_id npc, string_id response) throws InterruptedException
    {
        if (response.equals("force_trade"))
        {
            if (kyle_katarn_jediFriend_condition(player, npc))
            {
                final string_id message = new string_id(c_stringFile, "npc_offer_trade");

                utils.removeScriptVar(player, "conversation.kyle_katarn_conversation.branchId");
    ...
    public int kyle_katarn_handleBranch3(obj_id player, obj_id npc, string_id response) throws InterruptedException {
        ...
        if (response.equals("hireling_selection_01")) {
            reinforcementType = "jedi_reinforcement_1";
            factionCost = 1000;
        } else if (response.equals("hireling_selection_02")) {
            reinforcementType = "jedi_reinforcement_2";
            factionCost = 1000;
        } else if (response.equals("hireling_selection_03")) {
            reinforcementType = "jedi_reinforcement_3";
            factionCost = 1000;
        } else if (response.equals("hireling_selection_04")) {
            reinforcementType = "jedi_reinforcement_4";
            factionCost = 10000;
        } else if (response.equals("hireling_selection_05")) {
            reinforcementType = "jedi_reinforcement_5";
            factionCost = 18000;
        }
        ...
        if (reinforcementType != null) {
            ...
            kyle_katarn_action_grantHiring(player, npc, reinforcementType);
    ...
    public int kyle_katarn_handleBranch4(obj_id player, obj_id npc, string_id response) throws InterruptedException
    {
        if (response.equals("seek_balance2"))
        {
            if (kyle_katarn_chronicler_condition(npc, player))
            {
    ...
    public int kyle_katarn_handleBranch5(obj_id player, obj_id npc, string_id response) throws InterruptedException
    {
        if (response.equals("seek_jedi2"))
        {
            if (kyle_katarn_jediFriend_condition(npc, player))
            {
                ...
                responses[responseIndex++] = new string_id(c_stringFile, "seek_tat2");
                responses[responseIndex++] = new string_id(c_stringFile, "seek_cor1");

                utils.setScriptVar(player, "conversation.kyle_katarn_conversation.branchId", 6);
    ...
    public int kyle_katarn_handleBranch6(obj_id player, obj_id npc, string_id response) throws InterruptedException
    {
        if (response.equals("seek_tat1"))
        {

            final string_id message = new string_id(c_stringFile, "quest_tat1");
            kyle_katarn_jedi_quest1(player, npc);
        ...
        else if (response.equals("seek_tat2"))
        {

            final string_id message = new string_id(c_stringFile, "quest_tat2");
            kyle_katarn_jedi_quest2(player, npc);
        ...
        else if (response.equals("seek_cor1"))
        {
            ...
            kyle_katarn_jedi_quest3(player, npc);
    ...
    public int OnStartNpcConversation(obj_id npc, obj_id player) throws InterruptedException
    {
        ...
        if (kyle_katarn_language_condition(npc, player))
        {
            ...
            utils.setScriptVar(player, "conversation.kyle_katarn_conversation.branchId", 1);

            npcStartConversation(player, npc, "kyle_katarn_conversation", message, responses);
    ...
    {
        if (!conversationId.equals("kyle_katarn_conversation"))
        {
            return SCRIPT_CONTINUE;
        }

        final int branchId = utils.getIntScriptVar(player, "conversation.kyle_katarn_conversation.branchId");

        if (branchId == 1 && kyle_katarn_handleBranch1(player, npc, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        else if (branchId == 2 && kyle_katarn_handleBranch2(player, npc, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        else if (branchId == 3 && kyle_katarn_handleBranch3(player, npc, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        else if (branchId == 4 && kyle_katarn_handleBranch4(player, npc, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        else if (branchId == 5 && kyle_katarn_handleBranch5(player, npc, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        else if (branchId == 6 && kyle_katarn_handleBranch6(player, npc, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
