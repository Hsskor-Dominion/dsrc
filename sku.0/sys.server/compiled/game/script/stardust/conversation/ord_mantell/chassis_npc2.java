...
public class chassis_npc2 extends script.base_script
{
    public chassis_npc2()
    {
    }
    public static String c_stringFile = "conversation/chassis_npc";
    public boolean chassis_npc2_condition__defaultCondition(obj_id player, obj_id npc) throws InterruptedException
    {
        return true;
    }
    public boolean chassis_npc2_condition_hasLoot(obj_id player, obj_id npc) throws InterruptedException
    {
        return space_crafting.hasLootToSell(player);
    }
    public void vendor_jawa2_action_showTokenVendorUI(obj_id player, obj_id npc) throws InterruptedException
    {
    ...
    public void chassis_npc2_action_showComponentList(obj_id player, obj_id npc) throws InterruptedException
    {
    ...
    public void chassis_npc2_action_sellComponents(obj_id player, obj_id npc) throws InterruptedException
    {
    ...
    public String chassis_npc2_tokenTO_tokenTO0001(obj_id player, obj_id npc) throws InterruptedException
    {
        return new String();
    }
    public int chassis_npc2_handleBranch1(obj_id player, obj_id npc, string_id response) throws InterruptedException
    {
        if (response.equals("s_94e3013f"))
        {
            if (chassis_npc2_condition_hasLoot(player, npc))
            {
                chassis_npc2_action_sellComponents(player, npc);
            ...
            if (chassis_npc2_condition__defaultCondition(player, npc))
            {
        ...
        if (response.equals("s_buy"))
        {
            if (chassis_npc2_condition__defaultCondition(player, npc))
            {
                doAnimationAction(npc, "explain");
                vendor_jawa2_action_showTokenVendorUI(player, npc);
                string_id message = new string_id(c_stringFile, "s_ootini");
                utils.removeScriptVar(player, "conversation.chassis_npc.branchId");
                npcEndConversationWithMessage(player, message);
                return SCRIPT_CONTINUE;
            }
    ...
    {
        setCondition(self, CONDITION_CONVERSABLE);
        setName(self, "Het N'kek (Starship Parts Merchant)");
        setInvulnerable(self, true);
        setCondition(self, CONDITION_SPACE_INTERESTING);
        return SCRIPT_CONTINUE;
    }
    ...
    public int OnStartNpcConversation(obj_id self, obj_id player) throws InterruptedException
    {
        ...
        if (chassis_npc2_condition__defaultCondition(player, npc))
        {
            ...
            if (hasResponse)
            {
                ...
                if (hasResponse1)
                {
                    responses[responseIndex++] = new string_id(c_stringFile, "s_buy");
                }
                if (hasResponse2)
                {
    ...
    public int OnNpcConversationResponse(obj_id self, String conversationId, obj_id player, string_id response) throws InterruptedException
    {
        ...
        if (branchId == 1 && chassis_npc2_handleBranch1(player, npc, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
