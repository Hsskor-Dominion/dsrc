...
public class smuggler extends script.base_script
{
    ...
    public static void createRandomContrabandTier(obj_id player, int dropTier) throws InterruptedException
    {
        ...
        if (!isIdValid(pInv))
        {
            sendSystemMessageTestingOnly(player, "Invalid inventory — cannot create contraband.");
            return;
        }

        // --- STATIC CONTRABAND ITEM (always created) ---
        ...
        sendSystemMessage(player, new string_id("smuggler/messages", "contraband_found"));
    }
